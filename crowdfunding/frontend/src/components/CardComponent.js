...
const MyCard = (props) => {
	const { slug, collected_money, need_money, name, category, small_description, views } = props;
	return (
		<div>
			<Link to={`/projects/${slug}`} className="text-decoration-none">
				<Card className='border-0 rounded-3' style={{ backgroundColor: '#F5F5F5' }} >
					...
					<Card.Body className='d-flex flex-column'>
						...
						<Card.Text className='fs-5' style={{ height: '3em' }}>{small_description}</Card.Text>
						<div style={{ marginTop: 'auto' }}>
							<Card.Text>Собрано:</Card.Text>
							<ProgressBar completed={Math.round((collected_money / need_money) * 100)} completed_money={collected_money} need_money={need_money} />
							<div className='d-flex justify-content-between align-items-center'>
								...
								<div className='d-flex me-2 my-auto'>
									...
									<p className="d-flex align-items-center ms-1 align-middle my-auto fs-5">{views}</p>
								</div>
