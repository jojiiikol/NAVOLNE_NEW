...
const EditProfileModal = ({ show, onHide, username }) => {
    ...
    Object.keys(formData).forEach((key) => {
			if (key == "image") {
				...
				if (skills2.length !== 0) {
				skills2.forEach(cat => formDataObject.append('skill', cat));
				}
			}
			else{
			...
			<Modal.Body>
				<Form onSubmit={handleSubmit}>
					...
					<Form.Group className="mb-2">
						...
						<Form.Control as="textarea" rows={3} name="about" value={formData.about || ''} onChange={handleChange} placeholder="Расскажите немного о себе" />
					</Form.Group>
					...
					<Form.Group className="mb-2">
						...
						<Form.Control  type="number" name="passport" value={formData.passport || ''} onChange={handleChange} />
					</Form.Group>
					  ...
					  <Form.Group className="mb-3" >
								...
								{data === null ?
									...
									data.map((cat) => (
										<div key={cat.id}>
											<Form.Check type='checkbox' name={cat.id} onChange={handleChangeSkills} label={cat.name} />
										</div>
									))
								}
							</Form.Group>  
					...
					<Form.Group className="mb-2">
						...
						<Form.Control accept="image/jpeg,image/png,image/gif" name="image" type="file" value={formData.image || ''} onChange={handleChange} />
					</Form.Group>
